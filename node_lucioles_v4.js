...
async function v0() {
  ...
  const mongoUri =
    "mongodb+srv://iot:azerty@cluster0.tu2km.mongodb.net/myFirstDatabase?retryWrites=true&w=majority"; // connection  URI
  ...
  mg_client.connect(function (err, mg_client) {
    ...
    dbo.listCollections({ name: "json" }).next(function (err, collinfo) {
      if (collinfo) {
        // The collection exists
        //console.log('Collection temp already exists');
        dbo.collection("json").drop();
      }
    });
    ...
    client_mqtt.on("connect", function () {
      client_mqtt.subscribe(TOPIC_JSON, function (err) {
        if (!err) {
          //client_mqtt.publish(TOPIC_TEMP, 'Hello mqtt')
          console.log("Node Server has subscribed to ", TOPIC_JSON);
        }
      });
    });
    ...
    client_mqtt.on("message", function (topic, message) {
      ...
      var new_entry = {
        ...
        lat: lat, // latitude of the sensor
        longi: longi, // longitude of the sensor
